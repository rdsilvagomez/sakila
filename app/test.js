...
gr.addQuery('name','STARTSWITH','ns.')
gr.query(); 
while(gr.next())
{
    ...
    var gr_acl_role= new GlideRecord('sys_security_acl_role')
    gr_acl_role.addQuery('sys_user_role',gr.sys_id); 
    gr_acl_role.query(); 
	while(gr_acl_role.next()) 
	{
	 
		var gr_acl = new GlideRecord('sys_security_acl'); 
		gr_acl.get(gr_acl_role.sys_security_acl);
		//um.saveRecord(gr_acl); 
		//um.saveRecord(gr_acl_role); 
